...
options(digits = 3)
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning = FALSE, out.height = "50%", fig.width = 8, fig.height = 4.5, cache = TRUE)
library(ez)
library(haven)
library(knitr)
library(car)
...
-   Second dependent variable: **Issue involvement**
    ...
    -   To what extent should other people feel the issue of Tashkentistan becoming a member of the EU
    -   To what extent do you think the issue of whether Tashkentistan becomes a member of the EU warrants consideration?
...
-   Use OSF independently to find the data set, find the appropriate variables, and replicate the key analysis of the replication effect reported by Lemm: \> Replication of the key interaction result was unsuccessful. Participants were more persuaded overall by strong arguments than by weak arguments, F(1,247) = 13.01, p \< .001, partial eta-sq = .05, but this main effect was not moderated by the manipulation of value importance F(1,247) = .49, p = .49, partial eta-sq = .002. Consistent with the original article, the main effect of value importance was not significant, F(1,247) = .20, p = .66, partial eta-sq = .001.
...
-   You can find the file [here](https://osf.io/qifnv "Blankenship & Wegener (2008) replication data file").

- In jamovi, choose `ANOVA` from the `ANOVA` menu
- Put `Attitude` in the `Dependent Variable` field
- Put `arg_qual` and `value_imp` in the `Fixed Factors` field
- Tick "partial $\eta^2$" for effect size

# Output

```{r jmv_anova, echo=FALSE}
bw_anova <- ezANOVA(data = bw, dv = Attitude, wid = subID, between = .(arg_qual, value_imp), detailed = FALSE, return_aov = TRUE)

knitr::kable(bw_anova$ANOVA)
```

# Assumption tests

```{r assumptions}
resid(bw_anova$aov) %>% shapiro.test()

car::leveneTest(y = bw$Attitude, group = paste(bw$arg_qual, bw$value_imp))
```
# Plot

```{r bw_plot} 
bw_plot <- ggplot(data = bw, aes(x = arg_qual %>% as_factor, y = Attitude, colour = value_imp %>% as_factor, group = value_imp %>% as_factor)) + geom_jitter() + geom_smooth()#stat_summary(fun=mean, geom="line", aes(group = value_imp))
bw_plot
```
...
-   What does the finding mean for the original study?
    - Answer: The main finding, the interaction between argument quality and value importance, was not replicated.
    -   What does the finding mean for the hypothesis that persuasion attempts aimed at core values are more successful and long-lasting than those aimed at less important values?
        - It does not corroborate this hypothesis -- at the very least there should be another attempt at replication
-   Is there anything you should do about the assumption tests?
    - There is an apparent violation of the assumption of normality in the ANOVA residuals.
    - This may affect the power of the test, although likely not greatly since the ANOVA is quite robust to normality violations

...
-   Start by fitting a regression model on `positive_feelings` with just `fluffiness`
    -   You can do that in the `Linear Regression` module in jamovi.
        -   In SPSS, use `General Linear Model` → `Univariate...` so it is easier to add the factor later.
...
# Results: adding the contrast

```{r multiple_regression}
lm2 <- lm(data = fl, positive_feelings ~ fluffiness + animal)
tab_model(lm2, show.stat = TRUE)
fl$predicted2 <- predict(lm2)
```

# Results: adding the contrast (in words)

-   In words: `r report(lm2)`
    -   (note that this is a standard reporting template that was automatically generated, so you don't have to follow it exactly)
...
-   Consider the plot of the analysis of fluffiness alone:

```{r gaml_plots1, echo=FALSE}
p1  <- ggplot(fl, aes(x=fluffiness, y=positive_feelings, colour=animal))+
geom_point()+
scale_colour_discrete(name='Animal', labels=c('Cat','Dog'))+
geom_smooth(aes(x = fluffiness, y = predicted1), method = "lm", colour = 1) +
scale_linetype_discrete(name='Animal', labels=c('Cat','Dog'))+
labs(x = 'Fluffiness', y = 'Positive Feelings')
p1
```

# Answer (2)

-   And now the plot for the analysis including `animal`:

```{r gaml_plots2, echo=FALSE}
p2  <- ggplot(fl, aes(x=fluffiness, y=positive_feelings, colour=animal))+
geom_point()+
scale_colour_discrete(name='Animal', labels=c('Cat','Dog'))+
geom_line(aes(x = fluffiness, y = predicted2, linetype = animal)) +
scale_linetype_discrete(name='Animal', labels=c('Cat','Dog'))+
labs(x = 'Fluffiness', y = 'Positive Feelings')
p2
```
...
- As you can see, the line fitted when `animal` is not included is technically the best possible fit if you don't know that there are two groups, but in practice it ignores the trend we care about
- When we take `animal` into account, we can fit the line **within** the groups instead of **between**
- In this way, we can say that we have controlled for the effect of `animal` statistically -- we have removed the confound by including the confounding variable in the model
- Of course, it does not always work this nicely in reality!
- For example, the slope does not have to be the same for cats and dogs.
...
contrasts(fl$animal) <- contr.sum
lm3 <- lm(data = fl, positive_feelings ~ fluffiness + animal)
tab_model(lm3, show.stat = TRUE)
```
- The value of the coefficient always tells us how much the y-value changes when the x-value changes by 1
- When we use the 0 and 1 contrasts, that means the means for cat and dog are exactly 1 unit on the x-axis apart
    - That means 1 unit on the x-axis corresponds to 84 units on the y-axis
- When we use the -1 and 1 contrasts, that means the means for cat and dog are exactly 2 unit on the x-axis apart
    - That means 1 unit on the x-axis corresponds to 42 units on the y-axis

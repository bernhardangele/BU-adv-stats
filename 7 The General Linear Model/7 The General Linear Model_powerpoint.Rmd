---
title: "Advanced Statistics Lecture 7"
author: "The General Linear Model (GLM)"
...
write_sav(catfood_breed, path = "catfood_breed.sav")
```
...
$$Y_i = \alpha + \beta_1 X_i + \gamma_1 \times -1 + \delta_1(X_i \times -1) + \varepsilon_i = \alpha - \gamma_1 + (\beta_1 - \delta_1) X_i + \varepsilon_i$$
...
- $\beta_1$ is the average effect of CatWeight across breeds
...
colnames(contrast_matrix) <- c("$x_1$","$x_2$")
...
```{r, results='asis', echo=FALSE}
contrast_matrix <- contr.treatment(3)
colnames(contrast_matrix) <- c("$D_1$","$D_2$")
rownames(contrast_matrix) <- c("Beagle","Border Collie","Terrier")
kable(contrast_matrix)
```
...
- The predicted value for the Terrier group is $\alpha + \gamma_2$, i.e. the sum of the intercept and the second slope $\gamma_2$
...
qplot(data = dogs, y = objects, x = dog_iq, colour = breed) + geom_abline(intercept =  intercept, slope = slope, colour = "red") + geom_abline(intercept =  border_collie_intercept, slope = slope, colour = "green") + geom_abline(intercept =  terrier_intercept, slope = slope, colour = "blue")
```
